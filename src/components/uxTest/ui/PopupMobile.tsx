import React from 'react'
// CSS 파일 이름은 그대로 두거나, 원하시면 PopupMobileRefactored.css 등으로 변경하셔도 됩니다.
import './PopupMobile.css'
import { useAtomValue } from 'jotai'
import { modalAtom, setAtomValue } from '../../../utils/atom'

const PopupMobile = () => {
    const isModal = useAtomValue(modalAtom)
    return (
        <>
            {isModal &&
                // 모든 className 앞에 'mobile-' 접두사를 추가했습니다.
                <div className='mobile-background-div'
                    onClick={() => {
                        setAtomValue(modalAtom, false)
                    }}
                >
                    <div className="mobile-popup-wrapper">
                        <div className="mobile-popup-container">
                            <div className="mobile-image-section">
                                <img src="/lg.png" alt="BESPOKE 식기세척기" className="mobile-product-image" />
                                <div className="mobile-image-dots">
                                    <span className="mobile-dot active"></span>
                                    <span className="mobile-dot"></span>
                                    <span className="mobile-dot"></span>
                                </div>
                                <button className="mobile-close-button">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
                                    </svg>
                                </button>
                            </div>

                            <div className="mobile-info-section">
                                {/* 버튼이 잘리는 문제를 해결하기 위해 wrapper div 추가 */}
                                <div className="mobile-info-content-wrapper">
                                    <p className="mobile-category">BESPOKE 식기세척기</p>
                                    <h1 className="mobile-product-name">빌트인 14인용 화이트 글래스</h1>
                                    <p className="mobile-product-code">DW60BB815U01AP</p>

                                    <div className="mobile-features">
                                        <div className="mobile-feature-item">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.085a2 2 0 00-1.736.97l-2.714 4.222a2 2 0 00.175 2.323l3.109 3.109a2 2 0 002.828 0l7-7z" /></svg>
                                            <span>14인용 대용량 수납</span>
                                        </div>
                                        <div className="mobile-feature-item">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                                            <span>에너지 소비효율 1등급</span>
                                        </div>
                                        <div className="mobile-feature-item">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                                            <span>38분 Express 세척</span>
                                        </div>
                                    </div>

                                    <div className="mobile-price-section">
                                        <p className="mobile-price-label">가격</p>
                                        <p className="mobile-price-amount">1,490,000<span>원</span></p>
                                    </div>
                                </div>

                                <div className="mobile-button-group">
                                    <button className="mobile-button mobile-button-secondary">관심 상품</button>
                                    <button className="mobile-button mobile-button-primary">장바구니 담기</button>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            }
        </>
    )
}

export default PopupMobile
